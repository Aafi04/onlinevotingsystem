...
public class Dao {
    ...
    public static ResultSet adminValid(Model m) throws SQLException{
        // Modified to retrieve password_hash and salt based on username only
        // The comparison logic is moved to the servlet
        String sql="select adminId,username,password_hash,salt from admin where username=?";
    ...
    public static int registerAdmin(Model m) throws SQLException{
        int result =0;
        // Modified to store password_hash and salt instead of plaintext password
        // Assumes 'admin' table has 'password_hash' and 'salt' columns
        String sql="insert into admin(username,password_hash,salt) values(?,?,?)";
        PreparedStatement ps=con.prepareStatement(sql);
        ps.setString(1, m.getFullName());
        ps.setString(2, m.getPasswordHash()); // Assuming Model has getPasswordHash()
        ps.setString(3, m.getSalt());       // Assuming Model has getSalt()
